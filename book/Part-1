...
Представьте так называемый 'канал связи'. Вы должны отрыть соединение, послать сообщение, закрыть соединение. Это немалый объем работы особенно, когда отправляется большое количество сообщений по-одному. Однако мы может открыть канал всего раз, отправить все ожидающие соообщения и только тогда закрыть его.
...
React использует `ReactDefaultBatchingStrategyTransaction` (1). Как мы недавно поняли, самое главное в транзакции это его врапперы. Так что мы может взглянуть на эти врапперы и понять что именно определяет конкретная транзакция. Посмотрим на код: 
